...
9. **Треки на сервере не хранятся.** Античит устроен как ограничение скорости прироста площади
   именно поэтому: таблица точек с временными метками — более подробная история перемещений, чем
   маска, и заводить её ради проверки значит лечить одну утечку созданием худшей.
10. **Ветки кода, влияющие на время ответа при аутентификации, должны быть равными по стоимости.**
    Сверка пароля выполняется даже когда пользователя нет: иначе логины перечисляются секундомером.
...
./gradlew build
NVD_API_KEY=... ./gradlew dependencyCheckAnalyze   # известные CVE в зависимостях
```

Ключ NVD бесплатный, берётся на nvd.nist.gov. Без него первая выкачка базы уязвимостей занимает
часы и упирается в лимиты, поэтому в CI он обязателен.
